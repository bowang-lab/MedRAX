...
- **Automated Preliminary Report Generation**: NEW Provides structured preliminary reports for clinicians and simplified explanations for patients based on CXR analysis. Powered by the `AutomatedPreliminaryReportTool`.
...
MedRAX can generate two types of preliminary reports from chest X-ray images to assist both clinicians and patients:

1.  **Doctor-Focused Preliminary Report:**
    *   Provides a structured medical summary including findings and impressions derived from AI analysis of the CXR.
    *   This report is intended to give clinicians a quick overview and starting point for their own detailed review.

2.  **Patient-Focused Simplified Explanation:**
    *   Offers an explanation of the X-ray observations in simple, layperson's terms.
    *   This is designed to help patients better understand the results of their X-ray in preparation for a discussion with their doctor.
    *   Currently, this feature uses placeholder logic for common scenarios or can be configured to generate a prompt for a more detailed LLM-based explanation.

**How to Use:**
After uploading a CXR image, you can ask the MedRAX agent for a report. The agent may ask whether you need a detailed medical report (for a doctor) or a simplified explanation (for a patient).

**🚨 Important Considerations for Preliminary Reports:**
*   **AI-Generated and Preliminary:** All reports generated by this feature are based on AI models and should be considered preliminary. They are not a substitute for the expert interpretation and diagnosis provided by a qualified healthcare professional.
*   **Not a Diagnosis:** These reports, especially the patient-focused explanations, do **not** constitute a medical diagnosis.
*   **Consult Your Doctor:** Always discuss any X-ray results and reports with your doctor or other qualified healthcare provider. They will interpret the findings in the context of your overall health and determine any necessary next steps.
*   **Accuracy Limitations:** The accuracy of the reports is dependent on the underlying AI models and the quality of the input image.
